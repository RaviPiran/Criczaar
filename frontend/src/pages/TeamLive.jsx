...
// team stays "logged in" to its own code for this browser tab only
const storageKey = (roomCode) => `ca_team_session_${roomCode}`;

export default function TeamLive() {
  const { roomCode } = useParams();

  // ── Unlock state ──────────────────────────────────────────────────────
  const [myTeam,        setMyTeam]        = useState(null); // { teamId, name, color, logo, code }
  const [code,          setCode]          = useState('');
  const [unlocking,     setUnlocking]     = useState(false);
  const [unlockErr,     setUnlockErr]     = useState('');

  // ── Same state shape as the public LiveSpectator page ─────────────────
  ...
  // ── Load full room data (identical to LiveSpectator) ──────────────────
  const loadRoom = useCallback(async () => {
    try {
      ...
      // restore a saved team session for this room, if any
      const saved = sessionStorage.getItem(storageKey(roomCode));
  ...
  const connectSocket = (roomId) => {
    ...
    // Full state sync (emitted when a viewer first joins)
    ...
    // Admin made a live correction — refresh everything
  ...
  // ── Unlock with team code ──────────────────────────────────────────────
  const handleUnlock = async (e) => {
    e.preventDefault();
    if (!code.trim() || !room) return;
    setUnlocking(true); setUnlockErr('');
    try {
      const { data } = await PUB.post(`/teams/verify/${room._id}`, { code: code.trim() });
      if (!data.success) { setUnlockErr(data.message || 'Invalid code'); setUnlocking(false); return; }
      const session = { ...data.data, code: code.trim().toUpperCase() };
      sessionStorage.setItem(storageKey(roomCode), JSON.stringify(session));
      setMyTeam(session);
    } catch (err) {
      setUnlockErr(err.response?.data?.message || 'Invalid code');
    }
    setUnlocking(false);
  };
  ...
  // ── Derived stats (identical to LiveSpectator) ─────────────────────────
  ...
  // ── My team's live doc + bid math ──────────────────────────────────────
  const myTeamFull = myTeam ? teams.find(t => t._id === myTeam.teamId) : null;
  const getNextIncrement = (bid) => {
    const rule = room?.rules?.bidBonusRules?.find(r => bid >= r.minBid && bid <= r.maxBid);
    return rule?.bidIncrement ?? (room?.rules?.bidIncrement || 0.5);
  };
  const nextBid = currentPlayer ? Math.round((currentBid + getNextIncrement(currentBid)) * 10) / 10 : 0;
  const canBid  = !!currentPlayer && !isPaused && myTeamFull && nextBid <= (myTeamFull.budgetLeft ?? 0);

  const handleBid = async () => {
    if (!canBid || bidding) return;
    setBidding(true);
    try {
      await PUB.post(`/rooms/${room._id}/team-bid`, { teamId: myTeam.teamId, teamCode: myTeam.code, amount: nextBid });
      socketRef.current?.emit('place-bid', { roomId: room._id, teamId: myTeam.teamId, teamName: myTeam.name, amount: nextBid });
    } catch (err) {
  ...
  // ── Unlock gate (only blocks the Bid button — everything else visible) ──
  const UnlockCard = () => (
    <form onSubmit={handleUnlock} className="rounded-2xl border border-white/10 p-5 flex flex-col gap-3"
      style={{ background: 'rgba(255,255,255,0.04)' }}>
      <div className="text-xs text-white/40 uppercase tracking-widest">Bidding as a team? Unlock your team</div>
      <div className="flex gap-2 flex-wrap">
        <input
          ...
          maxLength={6}
          className="flex-1 min-w-[160px] bg-white/10 border border-white/20 rounded-xl px-4 py-2.5 text-center tracking-[0.3em] font-display outline-none focus:border-red-500"
        />
        <button disabled={unlocking} className="bg-red-600 hover:bg-red-700 disabled:opacity-50 rounded-xl px-5 py-2.5 font-display tracking-wide">
          {unlocking ? '…' : 'Unlock'}
        </button>
      </div>
      {unlockErr && <p className="text-red-400 text-xs">{unlockErr}</p>}
    </form>
  );
  ...
  return (
    <div className="min-h-screen text-white"
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] text-white/30 tracking-widest">TEAM BIDDING</div>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {myTeam && (
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border"
                style={{ background: `${myTeam.color}22`, color: myTeam.color, borderColor: `${myTeam.color}55` }}>
                {myTeam.name} <button onClick={handleLogout} className="ml-1 text-white/40 hover:text-white/80">✕</button>
              </span>
            )}
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-5">
        ...
        {/* ── Unlock (only until a team code is entered) ───────────────────── */}
        {!myTeam && <UnlockCard/>}
        ...
        {/* ── My team budget strip ─────────────────────────────────────────── */}
        {myTeam && myTeamFull && (
          <div className="rounded-2xl border border-white/8 p-4 flex flex-wrap gap-4 justify-between items-center"
            style={{ background: 'rgba(255,255,255,0.04)' }}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full overflow-hidden border-2 flex-shrink-0" style={{ borderColor: myTeam.color }}>
                {myTeam.logo ? <img src={myTeam.logo} alt={myTeam.name} className="w-full h-full object-cover"/>
                  : <div className="w-full h-full flex items-center justify-center font-display" style={{ background: myTeam.color }}>{myTeam.name?.[0]}</div>}
              </div>
              <div>
                <div className="font-display leading-none">{myTeam.name}</div>
                <div className="text-[10px] text-white/30 uppercase tracking-widest mt-1">Your team</div>
              </div>
            </div>
            <div className="flex gap-6">
              <div className="text-center">
                <div className="text-[10px] text-white/30 uppercase tracking-widest">Budget Left</div>
                <div className="font-orbitron font-bold text-emerald-400">{Math.round(myTeamFull.budgetLeft ?? 0)} pts</div>
              </div>
              <div className="text-center">
                <div className="text-[10px] text-white/30 uppercase tracking-widest">Slots Left</div>
                <div className="font-orbitron font-bold">{(myTeamFull.slots || 11) - (myTeamFull.players?.length || 0)}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {currentPlayer ? (
          <div className="rounded-3xl border border-white/10 overflow-hidden"
            ...
            <div className="p-6">
              ...
              {/* ── Bid button — only if this team is unlocked ─────────────── */}
              {myTeam && (
                <button
                  onClick={handleBid}
                  disabled={!canBid || bidding || isPaused}
                  className="w-full mt-5 rounded-2xl py-4 font-display text-lg tracking-wide disabled:opacity-40"
                  style={{ background: myTeam.color }}
                >
                  {bidding ? 'Placing…' : `Bid ${nextBid} pts`}
                </button>
              )}
              {myTeam && !canBid && !isPaused && (
                <div className="text-red-400 text-xs text-center mt-2">Not enough budget left for another bid</div>
              )}
            </div>
          </div>
        ) : lastResult ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {teams.map(team => {
              ...
              const maxBid = maxPlayerPrice
                ...
                : computedMax;
              const isMine = myTeam && team._id === myTeam.teamId;
              return (
                <div key={team._id} className="rounded-2xl overflow-hidden border"
                  style={{ background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(8px)', borderColor: isMine ? (team.color || '#dc2626') : 'rgba(255,255,255,0.08)' }}>
                  ...
                  <div className="p-5">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-display text-lg text-white truncate flex items-center gap-2">
                          {team.name} {isMine && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/10 border border-white/20">YOU</span>}
                        </div>
